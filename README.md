# Qwen3 Ollama RAG Experiments
My experiments with creating multilingual local RAG systems using Ollama and the Qwen3 embeddings and rerankers. Both text-only and multimodal RAG.

<br>

## Experiments


<br>

## Resources

- Gemini discussion<br>
https://gemini.google.com/app/cdf34b20b40a1a8c?utm_source=app_launcher&utm_medium=owned&utm_campaign=base_all

<br>

- Qwen3-VL-Embedding<br>
https://huggingface.co/collections/Qwen/qwen3-vl-embedding

- Qwen3-VL-Reranker<br>
https://huggingface.co/collections/Qwen/qwen3-vl-reranker

- Blog post<br>
Qwen3-VL-Embedding and Qwen3-VL-Reranker: For the Next Generation of Multimodal Retrieval<br>
https://qwen.ai/blog?id=qwen3-vl-embedding

<br>

- Qwen3-Embedding<br>
https://huggingface.co/collections/Qwen/qwen3-embedding

- Qwen3-Reranker<br>
https://huggingface.co/collections/Qwen/qwen3-reranker

- Blog post<br>
Qwen3 Embedding: Advancing Text Embedding and Reranking Through Foundation Models<br>
https://qwen.ai/blog?id=qwen3-embedding

<br>

- Qwen3 Multimodal Embeddings: Finally, RAG That Sees<br>
Sam Witteveen<br>
https://www.youtube.com/watch?v=fY3-YeveBgA

- Qwen 3 Embeddings & Rerankers<br>
Sam Witteveen<br>
https://www.youtube.com/watch?v=hize6rD6Afk

<br>

- Part 1 - Build an ArXiv RAG search system w FAISS<br>
  https://www.kaggle.com/code/vbookshelf/part-1-build-an-arxiv-rag-search-system-w-faiss
  
- Part 2 - Search ArXiv using text vector similarity<br>
  https://www.kaggle.com/code/vbookshelf/part-2-search-arxiv-using-text-vector-similarity

<br>

  ## Rough Notes

  - Ollama supports creating text embeddings but there's no support for using reranking models.
  - Ollama does not support creating multimodal embeddings i.e. text + image
  - If you have hundreds of thousands of text embeddings to create, local inference with Ollama is too slow to be practical. (Tested on M4 Macbook Air)
  - The Qwen3 0.6B Embedding model available on Kaggle tends to produce low quality results compared to when the model is downloaded from Hugging face using the Sentence Transformers package. I'm not sure if the Kaggle model is full precision or a quantized version. Tested using the data from this competition:<br>
  LLM Agentic Legal Information Retrieval<br>
  https://www.kaggle.com/competitions/llm-agentic-legal-information-retrieval
- Qwen3 embedding models support Matryoshka Embeddings. If you manually slice the embeddings to reduce their length, it's important to normalize your sliced embeddings. Refer to the reduce_embedding_dim function under the Embedding Dimension Reduction (MRL) section of this Colab notebook by Sam Witteveen:<br>
https://colab.research.google.com/drive/1R1WEX2KJ5for89vsNq00AR6RzhrdTHYw?usp=sharing#scrollTo=dim_reduction

